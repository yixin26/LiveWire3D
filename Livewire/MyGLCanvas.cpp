...
void MyGLCanvas::keyPressEvent(QKeyEvent *e)
{
	...
	case Qt::Key_Backspace:
		if (latestOperationMode == DEL) {
			m_pathUtils->deleteCurrentCurve();
			latestOperationMode = OFF;
		}
		else if (latestOperationMode == NORMAL) {
			m_pathUtils->clearCurve();
			latestOperationMode = OFF;
		}
		else if (latestOperationMode == EDIT) {
			//m_pathUtils->popUserDrawCurves();
			latestOperationMode =OFF;
		}
		break;
	}
...
void MyGLCanvas::mousePressEvent(QMouseEvent *event)
{
	...
	if (m_interaction->isControlPress)
	{
		if (event->button() == Qt::LeftButton) 
			m_pathUtils->pickMeshVertex(m_lastx, m_lasty, 1, true);
		else if (event->button() == Qt::RightButton)
			m_pathUtils->clearCurve();
	}
	else if (m_interaction->isAltPress)
	{
		if (event->button() == Qt::LeftButton) 
			m_pathUtils->pickCurve(m_lastx, m_lasty, true);
	}
	else if (m_interaction->isShiftPress &&false)
	{
...
void MyGLCanvas::mouseReleaseEvent(QMouseEvent *event) 
{
	...
	if (m_interaction->isShiftPress && false) //scribbling
	{
...
void MyGLCanvas::wheelEvent(QWheelEvent *e)
{
	...
	else
	{
		if (m_interaction->isControlPress)
		{
			std::vector<bool> isDo(8, false);
			if (m_pathUtils->isDoEuclideanGeodesy)		isDo[0] = true;
			if (m_pathUtils->isDoOursGeodesy1)			isDo[1] = true;
			if (m_pathUtils->isDoOursGeodesy2)			isDo[2] = true;
			if (m_pathUtils->isDoPottmannsGeodesy)		isDo[3] = true;
			if (m_pathUtils->isDoKovacsGeodesy1)		isDo[4] = true;
			if (m_pathUtils->isDoKovacsGeodesy2)		isDo[5] = true;
			if (m_pathUtils->isDoCampensGeodesy1)		isDo[6] = true;
			if (m_pathUtils->isDoCampensGeodesy2)		isDo[7] = true;

			while (true)
			{
				int type;
				for (type = 0; type < 8; type++)
					if (m_pathUtils->m_anisAlgo == MetricType(type))
						break;
				if (e->delta() > 0)
					type = (type + 1) % 8;
				else
				...
				m_pathUtils->m_anisAlgo = MetricType(type);
				if (isDo[type]) break;
			}
		}
	}
...
void MyGLCanvas::processSketch(QMouseEvent *event)//mouse move
{
	...
	if (m_interaction->isAltPress) //selection
	{
		latestOperationMode = DEL;

		m_pathUtils->pickCurve(x, y, false);
	}
	else if (m_interaction->isControlPress) //normal
	{
		latestOperationMode = NORMAL;
		...
		m_pathUtils->pickMeshVertex(x, y, 1, false);
	}
	else if (m_interaction->isShiftPress && false) //scribbling
	{
		latestOperationMode = EDIT;
		...
		if (isLeftDown && !(m_selectx == x&&m_selecty == y)) 
		{
			if (m_interaction->sr == Interaction::PIXEL)
			{
...
void MyGLCanvas::ScreenShot()
{
	...
	if (bmpBuffer != NULL) {
		...
		QString imageFile = m_pathUtils->m_filename;
		imageFile.replace(imageFile.lastIndexOf(".") + 1, 3, "bmp");
		FILE *filePtr = fopen(imageFile.toStdString().data(), "wb");
		std::cout << "save screen to " << imageFile.toStdString().data() << std::endl;
